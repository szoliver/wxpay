; (function ($, window) {
    ...
    $.fn.wxPay = function (signurl, openid, options, paybefore, success, fail, cancel) {
        ...
        $(document).on("success",success);
        $(document).on("fail",fail);
        $(document).on("cancel",cancel);
                                    ...
                                    if (data.paySign == "ERROR") {
                                        ...
                                        'getBrandWCPayRequest', {
                                            ...
                                            if (res.err_msg == "get_brand_wcpay_request:ok") {
                                                if (typeof success === "function") $(document).trigger('success', {fee:feeamt,ordercode:orderstr});
                                            } else
                                                if (res.err_msg == "get_brand_wcpay_request:fail") {
                                                    if (typeof fail === "function") $(document).trigger('fail', res.err_desc);
                                                } else {
                                                    if (typeof cancel === "function") $(document).trigger('cancel', data.payNo);
                                                }
                                        });
                                    }
                                }, "json");
